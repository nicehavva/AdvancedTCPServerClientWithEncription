...
namespace Nicehavva.AdvancedTCP.Server
{
    public class Receiver
    {
            ...
            : this()
        {
            ...
            Client.SendBufferSize = 1024 * 1024 * 1;
        }
        ...
        {
            foreach (var client in Server.Receivers.Where(x => x != this && x.Status != StatusEnum.Disconnected))
            {
                DisconnectRequest msg = new DisconnectRequest();
                msg.SenderClient = this.ClientName;
        ...
        private void OnMessageReceived(MessageBase msg)
        {
            ...
            {
                foreach (var client in Server.Receivers.Where(x => x != this && x.Status != StatusEnum.Disconnected && x.ClientName == msg.ReceiverClient))
                {
                    msg.SenderClient = this.ClientName;
                    client.SendMessage(msg);
                }
        ...
        private void SessionRequestHandler(SessionRequest request)
        {
            ...
            foreach (var receiver in Server.Receivers.Where(x => x != this && x.Status==StatusEnum.Validated))
            {
                if (receiver.ClientName == request.ClientName)
                {
            ...
            response.IsConfirmed = false;
            response.HasError = true;
            response.Exception = new Exception(request.ClientName + " does not exists or not loged in or in session with another user.");
            SendMessage(response);
        }
        ...
        private void ValidationRequestHandler(ValidationRequest request)
        {
            ...
            () =>
            {
                ...
                response.HasError = true;
                response.Exception = new AuthenticationException("Login failed for user " + request.ClientName);
                SendMessage(response);
            });
